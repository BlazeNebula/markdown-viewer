...
- **AUTO 宽度模式**：内容宽度实时自适应侧边栏可见宽度（`100vw - 侧边栏宽度`），
  随窗口缩放和 TOC 收起/展开实时无级变化，不再使用固定断点
- **层级虚线样式**：TOC 层级链添加虚线连接，支持悬停高亮效果，
  使用 `box-shadow` 外阴影提升布局稳定性，适配光暗模式
- **快速同步脚本**：新增 `scripts/快速同步.ps1` 用于开发调试

### 变更

- **构建与测试文档**：新增构建与测试 prompt、新编译器 prompt，
  更新版本发布工作流说明（SKILL.md）
...
- **TOC 侧边栏切换按钮**：点击 ☰ 按钮收起/展开目录，无需进入设置开关
- **TOC 侧边栏动画**：收起/展开时平滑滑入滑出（transform GPU 加速）
- **主视图联动画**：侧边栏收展时，内容区域按比例平移
  （平移量 = TOC宽度 × 内容宽度 ÷ 视窗宽度），不改变内容排版
- **侧边栏背景自适应**：跟随主题背景色，不再透明重叠

### 修复

- 侧边栏 z-index 提升，确保不被内容覆盖
- 大量 Mithril 选择器语法修正（`.class1 class2` → `.class1.class2`）
